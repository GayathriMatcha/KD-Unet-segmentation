...
from models import TUNet,SUNet,DCStudentUNet
...
if __name__ == "__main__":
    ...
    object_type = 'cardiac'
    model_type = 'DCStudentUNet'
    save_path = '/home/hticimg/gayathri/Context-aware-segmentation/KD_seg/{}_{}_SFTN/models_local'.format(object_type,model_type)

    use_pretrained = True
    pretrained_model_path = '/home/hticimg/gayathri/Context-aware-segmentation/KD_seg/cardiac_SFTN/models_local/150.pt'.format(object_type,model_type)
    ...
    writer = SummaryWriter(log_dir='/home/hticimg/gayathri/Context-aware-segmentation/KD_seg/{}_{}_SFTN/models_local/summary'.format(object_type,model_type))

    logging.basicConfig(filename="log_{}_run_local_{}_SFTN.txt".format(object_type,model_type),
                            filemode='a',
                            format='%(asctime)s,%(msecs)d %(name)s %(levelname)s %(message)s',
                            datefmt='%Y-%m-%d %H:%M',
                            level=logging.INFO)

    logging.info('Model: DCStudentUNet + Loss:  {}'.format(object_type)) 
...
#         print(checkpoint)
        ...
        for mw in model_wts:
#             print(f'model weights are: {mw}')
            for tw in checkpoint:
