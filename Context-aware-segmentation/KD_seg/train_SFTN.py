...
#     criterion = FocalLoss(num_classes=4,device=args.device)  #FOCAL
    ...
    for iter, data in enumerate(loop):
        ...
        with torch.set_grad_enabled(True):
            ...
            loss_teacher = criterion(outputs[-1],targets)
            loss_student = criterion(outputs[0],targets)+ criterion_global(outputs[1],targets)
            loss_imitation = kl_loss(outputs[0],outputs[-1])+kl_loss(outputs[1],outputs[-1])
                
            loss= loss_teacher+ (1/2)*loss_student + (1/2)*loss_imitation
...
#         model = UNet(num_classes=4,input_channels=1,down_filter_factors=(1, 2, 4),up_filter_factors=(1, 2, 4)).to(args.device)
    if args.model_type == 'UNet_T':
        model = UNet(num_classes=4,input_channels=1,down_filter_factors=(1, 2, 4, 8),up_filter_factors=(1, 2, 4, 8)).to(args.device)
