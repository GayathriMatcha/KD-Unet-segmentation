...
def train_epoch(args, epoch,model,data_loader, optimizer, writer):
    ...
    div = DistillKL(T=1)
#     criterion = LossMulti(num_classes=4,jaccard_weight=0,device=args.device)
#     kl_loss = nn.KLDivLoss(reduction="batchmean", log_target=True)### CE loss
...
#     criterion = FocalLoss(num_classes=4,device=args.device)  #FOCAL
    ...
    for iter, data in enumerate(loop):
        ...
        with torch.set_grad_enabled(True):
            ...
            loss_teacher = criterion(outputs[-1], targets.squeeze(1)) + dice_loss(F.softmax(outputs[-1], dim=1).float(),F.one_hot(targets.squeeze(1),4).permute(0, 3, 1, 2).float(), multiclass=True)
            
            loss_student = criterion(outputs[0], targets.squeeze(1)) + criterion(outputs[1], targets.squeeze(1)) + dice_loss(F.softmax(outputs[0], dim=1).float(),F.one_hot(targets.squeeze(1),4).permute(0, 3, 1, 2).float(), multiclass=True) + dice_loss(F.softmax(outputs[1], dim=1).float(),F.one_hot(targets.squeeze(1),4).permute(0, 3, 1, 2).float(), multiclass=True)
            
            loss_imitation = div(outputs[0],outputs[-1])+ div(outputs[1],outputs[-1])       
                
            loss= loss_teacher+ 0.5*(1/2)*loss_student +  10*(1/2)*loss_imitation
#             print(f'loss_T {loss_teacher} loss_S {loss_student/2} loss_KLdiv {loss_imitation/2}')
...
def evaluate(args, epoch, model,data_loader, writer):
    ...
    with torch.no_grad():
        for iter, data in enumerate(tqdm(data_loader)):
            ...
            loss = criterion(outputs, targets.squeeze(1)) + dice_loss(F.softmax(outputs, dim=1).float(),F.one_hot(targets.squeeze(1),4).permute(0, 3, 1, 2).float(), multiclass=True)
...
#     logging.info('Model: DCStudentUNet + Loss:  {}'.format(args.object_type)) 
    ...
    if args.model_type == 'studentSFTN':
        ...
        model = load_model(model,args.teacher_checkpoint['model'])
    #print (model)
