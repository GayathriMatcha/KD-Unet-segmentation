...
from kd_methods import HintLoss, Attention, Similarity, DistillKL
...
#             loss_main = criterion(outputS[-1], targets.squeeze(1)) + dice_loss(F.softmax(outputS[-1], dim=1).float(),F.one_hot(targets.squeeze(1),4).permute(0, 3, 1, 2).float(), multiclass=True)
            ...
            loss= loss_kd#1*loss_main + 1* loss_div #+ 0.1 * loss_kd
#             print(f'losses are main {loss_main} and kl div loss {loss_div} and hintloss is {loss_kd}')
