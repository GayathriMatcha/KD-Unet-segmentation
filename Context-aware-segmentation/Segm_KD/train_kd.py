...
from kd_methods import HintLoss, Attention, Similarity, DistillKL, Correlation
...
#     kd = Similarity()
    kd = Correlation()
    alpha = 1 #weight for original CE loss
    beta = 10 #weight for KL div between teacher and student 
    gama = 0 # weight for other distillation losses
...
#             outputS_feat = [outputS[0],outputS[1]]
            ...
            loss_div = div(outputS[-1],outputT[-1])
            
            loss_main = criterion(outputS[-1], targets.squeeze(1)) + dice_loss(F.softmax(outputS[-1], dim=1).float(),F.one_hot(targets.squeeze(1),4).permute(0, 3, 1, 2).float(), multiclass=True)
            
#             loss_group = kd(outputS_feat,outputT_feat)
#             loss_kd = sum(loss_group)
            loss_kd = kd(outputS[1],outputT[1])
#             print(f'Attentionloss is {loss_kd}')

            
            
                
            loss= 1*loss_main + 1* loss_div + 1* loss_kd #[100 for at, 10 for sp]
#             print(f'losses are main {loss_main} and kl div loss {loss_div} and Attentionloss is {loss_kd}')
...
#             outputT = modelT(inputs)
            ...
            loss = criterion(outputS[-1], targets.squeeze(1)) + dice_loss(F.softmax(outputS[-1], dim=1).float(),F.one_hot(targets.squeeze(1),4).permute(0, 3, 1, 2).float(), multiclass=True)
