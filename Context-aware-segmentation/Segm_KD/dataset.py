...
class DatasetImageMaskLocal(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(img_file_name, 'r') as data:
            ...
            coord = data['coord'][:]
#             print(f'co ord value is {coord}')
#             print(f' this is the data {data['coord']} exactly')
#             print(f'co ord value is {torch.from_numpy(coord)}')
...
class DatasetImageMaskLocaldev(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_file_name = self.file_names[idx]
        img_file_name = pathlib.Path(img_file_name)
...
#             print(f'co ord value is {torch.from_numpy(coord)}')
        ...
        if self.mode == 'train':    
            return torch.from_numpy(image).float(),torch.from_numpy(mask).long(),torch.from_numpy(coord),str(img_file_name.name)
        if self.mode == 'valid':
            return torch.from_numpy(image).float(),torch.from_numpy(mask).long(),str(img_file_name.name)