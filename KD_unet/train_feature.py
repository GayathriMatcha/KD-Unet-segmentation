...
#         outputS_feat = [outputS[0][1],outputS[1][1],outputS[2][1],outputS[3][1],outputS[4][1]]
        # last layer 
        outputT_feat = [outputT[0],outputT[1],outputT[2],outputT[3],outputT[4]]
        outputS_feat = [outputS[0][-1],outputS[1][-1],outputS[2][-1],outputS[3][-1],outputS[4][-1]]
...
def evaluate(args, epoch, modelT,modelS,data_loader, writer):
    ...
    with torch.no_grad():
        for iter, data in enumerate(tqdm(data_loader)):
            ...
            outputT = modelT(input,input_kspace,mask)
            outputS = modelS(input,input_kspace,mask)
#             print(f'teacher output is {len(outputT)}')
#             print(f'teacher output is {len(outputS)}')
...
#             outputS_feat = [outputS[0][1],outputS[1][1],outputS[2][1],outputS[3][1],outputS[4][1]]
        ...
        # last layer 
            outputT_feat = [outputT[0],outputT[1],outputT[2],outputT[3],outputT[4]]
            outputS_feat = [outputS[0][-1],outputS[1][-1],outputS[2][-1],outputS[3][-1],outputS[4][-1]]
#             print(f'outputT[0] shape is {outputT[0].shape},{outputS[0][-1].shape}')
