...
def create_datasets_knee(args):
    ...
    train_data = KneeData(args.train_path,acc_factors,dataset_types,mask_types,'train')
    dev_data = KneeData(args.validation_path,acc_factors,dataset_types,mask_types,'validation')
...
def train_epoch(args, epoch,model,data_loader, optimizer, writer):
    ...
    for iter, data in enumerate(loop):
        ...
        output = model(img_und,rawdata_und,masks,sensitivity,img_und.shape)
#         op = output[-1].permute(0,2,3,1)
        img_gt = T.complex_center_crop(img_gt,(320,320))
        img_gt = img_gt.permute(0,3,1,2)
 
        loss1 = F.l1_loss(output[-1],img_gt)
    
        loss2 = F.l1_loss(output[0],img_gt)+F.l1_loss(output[1],img_gt)+F.l1_loss(output[2],img_gt)+F.l1_loss(output[3],img_gt)
...
def evaluate(args, epoch, model,data_loader, writer):
    ...
    with torch.no_grad():
        for iter, data in enumerate(tqdm(data_loader)):
            ...
            output = model(img_und,rawdata_und,masks,sensitivity,img_und.shape)[-1]
            output = output.permute(0,2,3,1) 
            img_gt = T.complex_center_crop(img_gt,(320,320))

            loss = F.l1_loss(output,img_gt)
...
#             output = output.permute(0,2,3,1)
            ...
            output  = T.complex_abs(output)
             
            img_und  = img_und .unsqueeze(1)
            img_gt   = img_gt.unsqueeze(1)
            output   = output.unsqueeze(1)
            
            save_image(img_und , 'Input')
            save_image(img_gt , 'Target')
            save_image(output , 'Reconstruction')
            save_image(torch.abs(output  - img_gt ), 'Error')
...
def create_arg_parser():
    ...
    parser.add_argument('--report-interval', type=int, default=200, help='Period of loss reporting')
