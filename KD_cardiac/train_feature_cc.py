...
#         print(f'output_T is {len(outputT_feat)}')
        loss_t = [correlaion_congruence(s,t) for s, t in zip(outputT_feat,outputS_feat)]
#        loss_t = [sim_dis_compute(s,t) for s, t in zip(outputT_feat,outputS_feat)]
#         loss_t = [CriterionPairWiseforWholeFeatAfterPoolFeatureMaps(s,t) for s, t in zip(outputT_feat,outputS_feat)]
       # print(one)
        loss   = reduce((lambda x,y : x + y),loss_t)  / len(loss_t)
#         print(f'loss total is {loss_t}')
