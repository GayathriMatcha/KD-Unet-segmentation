...
from trial1 import DCTeacherNet,DCStudentNet #,Vgg16
import torchvision
...
# # #         print(f'outputT_feat[0].shape-->1 is {outputT_feat[0].shape}')
        
#         outputT_feat = [torch.sum(x**2,dim=1) for x in outputT_feat]
#         outputS_feat = [torch.sum(x**2,dim=1) for x in outputS_feat]
# #         print(f'outputT_feat[0].shape-->2 is {outputT_feat[0].shape}')
...
#         loss_t = [similarity_loss(s,t) for s, t in zip(outputT_feat,outputS_feat)]
#         loss   = reduce((lambda x,y : x + y),loss_t)  / len(loss_t)
#         print(f'length of s_fsp: {loss_group}')
