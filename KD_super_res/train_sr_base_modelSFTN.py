...
from models import TeacherVDSR,StudentVDSR,SFTN3
import torchvision
...
#         target = target.unsqueeze(1).to(args.device)
        ...
        loss2 = F.l1_loss(output[0],target)+F.l1_loss(output[1],target)+F.l1_loss(output[2],target)
        
        loss3 = F.l1_loss(output[0],output[-1])/output[-1].numel()+ F.l1_loss(output[1],output[-1])/output[-1].numel() +F.l1_loss(output[2],output[-1])/output[-1].numel()
        
#         loss = F.l1_loss(output[-1],target)
        loss = loss1 + 1*(loss2/2) + 1*(loss3/2) 
